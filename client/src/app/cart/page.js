...
export default function CartPage() {
    ...
    if (loading && (!cart.items || cart.items.length === 0)) {
        return (
            <div className="container mx-auto px-4 py-8 bg-[#FAF6F0] min-h-screen">
                <div className="mb-8">
                    <h1 className="text-3xl md:text-4xl font-display font-bold tracking-wide text-[#6A1E1E] mb-2">
                        Your Cart
                    </h1>
    ...
    if ((!cart.items || cart.items.length === 0) && !error) {
        return (
            <div className="container mx-auto px-4 py-12 bg-[#FAF6F0] min-h-screen">
                <div className="mb-8">
                    <h1 className="text-3xl md:text-4xl font-display font-bold tracking-wide text-[#6A1E1E] mb-2">
                        Your Cart
                    </h1>
                ...
                <div className="bg-[#FFFEF9] p-8 md:p-12 rounded-md shadow-md text-center border border-[#D4AF37]/20 max-w-2xl mx-auto">
                    <div className="inline-flex justify-center items-center bg-[#D4AF37]/10 p-6 rounded-md mb-6">
                        <ShoppingBag className="h-16 w-16 text-[#D4AF37]" />
                    </div>
                    <h2 className="text-2xl font-semibold mb-3 text-[#6A1E1E]">
                        Your cart is empty
                    </h2>
                    <p className="text-[#7B2D26]/70 mb-8 text-lg">
                        Looks like you haven&apos;t added any delicious snacks yet.
                    </p>
                    <Link href="/products">
                        <Button className="bg-[#D4AF37] hover:bg-[#C4A030] text-[#4A1515] px-8 py-6 text-base font-semibold shadow-lg hover:shadow-xl transition-all duration-200 rounded-md">
    ...
    return (
        <div className="container mx-auto px-4 py-8 bg-[#FAF6F0] min-h-screen">
            ...
            {!isAuthenticated && cart.items.length > 0 && (
                <div className="bg-[#D4AF37]/10 border-l-4 border-[#D4AF37] p-6 rounded-md flex items-start mb-6 shadow-md hover:shadow-lg transition-shadow duration-300">
                    <div className="flex-shrink-0 mr-4">
                        <div className="w-14 h-14 bg-[#D4AF37] rounded-md flex items-center justify-center shadow-lg">
                            <svg
                                className="w-7 h-7 text-[#4A1515]"
                                fill="none"
                                stroke="currentColor"
                                viewBox="0 0 24 24"
                            >
                    ...
                    <div className="flex-1">
                        <h2 className="text-xl font-bold text-[#6A1E1E] mb-2 flex items-center gap-2">
                            Guest Shopping Cart
                            <span className="text-xs font-normal bg-[#D4AF37]/20 text-[#6A1E1E] px-2 py-1 rounded-md">
                                Login Required
                            </span>
                        </h2>
                        <p className="text-[#7B2D26]/80 mb-4 leading-relaxed">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                            <Link href="/auth?redirect=cart">
                                <Button className="bg-[#D4AF37] hover:bg-[#C4A030] text-[#4A1515] font-semibold px-6 py-3 rounded-md shadow-md hover:shadow-xl transition-all duration-200 transform hover:scale-[1.02]">
                                    Log In to Continue
                                </Button>
                            </Link>
                            <Link href="/auth?redirect=cart">
                                <Button
                                    variant="outline"
                                    className="border-2 border-[#D4AF37] hover:bg-[#D4AF37] hover:text-[#4A1515] text-[#6A1E1E] font-semibold px-6 py-3 rounded-md transition-all duration-200"
                                >
                                    Create Account
                                </Button>
                            </Link>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1">
                    <div className="bg-[#FFFEF9] rounded-md shadow-md border border-[#D4AF37]/20 p-5 md:p-6 sticky top-4 hover:shadow-lg transition-shadow duration-300">
                        <div className="mb-6">
                            <h2 className="text-xl font-bold text-[#6A1E1E] mb-2">
                                Cart Summary
                            </h2>
                        ...
                        {/* Price Details */}
                        <div className="border-t-2 border-[#D4AF37]/30 pt-5">
                            <div className="space-y-3 text-sm">
                                <div className="flex justify-between items-center py-1">
                                    <span className="text-[#7B2D26]/80 font-medium">Subtotal</span>
                                    <span className="font-semibold text-[#6A1E1E]">
                                        {!isAuthenticated && hidePricesForGuests ? (
                                            <Link href="/auth?redirect=cart" className="text-[#D4AF37] hover:underline"> Login to view</Link>
                                        ) : (formatCurrency(totals.subtotal))}
                                    </span>
                                </div>

                                {coupon && (
                                    <div className="flex justify-between items-center bg-[#1A7F37]/10 px-3 py-2 rounded-md">
                                        <span className="text-[#1A7F37] font-medium">Discount</span>
                                        <span className="text-[#1A7F37] font-bold">
                                ...
                                <div className="flex justify-between items-center py-1">
                                    <span className="text-[#7B2D26]/80 font-medium">Shipping</span>
                                    {totals.shipping > 0 ? (
                                        <span className="text-[#6A1E1E] font-bold">
                                            {formatCurrency(totals.shipping)}
                                        </span>
                                    ) : (
                                        <span className="text-[#1A7F37] font-bold text-base flex items-center gap-1">
                                            <svg
                                                className="w-4 h-4"
                                                fill="none"
                                                stroke="currentColor"
                                ...
                                {totals.shipping > 0 && cart.freeShippingThreshold > 0 && (
                                    <div className="mt-2 text-sm text-[#D4AF37] bg-[#D4AF37]/10 p-3 rounded-md text-center font-medium border border-[#D4AF37]/20 flex items-center justify-center gap-2">
                                        <svg className="w-5 h-5 text-[#D4AF37]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                        </svg>
                                        <span>
                                            Add <strong>{formatCurrency(cart.freeShippingThreshold - totals.subtotal)}</strong> more for <span className="text-[#1A7F37] font-bold">FREE shipping!</span>
                                        </span>
                                    </div>
                                )}
                            ...
                            <div className="flex justify-between items-center font-bold text-xl mt-5 pt-5 border-t-2 border-[#D4AF37]/30 bg-[#6A1E1E] px-4 py-4 rounded-md">
                                <span className="text-[#FAF6F0]">Total</span>
                        ...
                        {/* Checkout Button */}
                        <Button
                            className="w-full mt-6 bg-[#D4AF37] hover:bg-[#C4A030] text-[#4A1515] shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-[1.02] font-semibold py-6 text-base rounded-md"
                            size="lg"
                        ...
                        <div className="mt-4 flex items-center justify-center gap-2 text-xs text-[#7B2D26]/60">
                            <svg
                                className="w-4 h-4"
                                fill="none"
                                stroke="currentColor"
